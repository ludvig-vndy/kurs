...
export function renderDagsbrev({ datum, poster, lugna }) {
  const s = {
    ...
    h2: 'font-family:Georgia,serif;font-size:16px;margin:0 0 6px;color:#211C17',
  ...
  <div style="${s.sek}">
    ...
    <p style="${s.p}"><a style="${s.a}" href="${post.url}">Källdokumentet →</a></p>
  </div>`).join('');
  ...
  <div style="max-width:600px;margin:0 auto">
    ...
    <h1 style="font-family:Georgia,serif;font-size:26px;color:#211C17;margin:14px 0 4px">Ägarbrevet</h1>
    <p style="font-size:13px;color:#5C544A;margin:0 0 18px">${datum} · ${poster.length} ${poster.length === 1 ? 'sak' : 'saker'} i dina bolag i natt. Varje siffra bär citat ur källdokumentet.</p>
    ${sektioner || `<div style="${s.sek}"><p style="${s.p}">Inget nytt i något bevakat bolag. Det är ett besked, inte ett fel.</p></div>`}
    ${lugna.length ? `<div style="${s.sek}"><p style="${s.et}">Lugnt</p><p style="${s.p}">Inget nytt i: ${lugna.join(', ')}.</p></div>` : ''}
