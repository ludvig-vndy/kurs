...
const modeArg=arg('--mode','both');if(!['both','pilot','baseline','pilot-cache','cache-pair'].includes(modeArg))throw Error('Invalid mode');
...
try{
 outer:for(let repeat=1;repeat<=repeats;repeat++)for(const c of jobs)for(const mode of modeArg==='both'?(repeat%2?['baseline','pilot']:['pilot','baseline']):modeArg==='cache-pair'?(repeat%2?['pilot','pilot-cache']:['pilot-cache','pilot']):[modeArg]){
