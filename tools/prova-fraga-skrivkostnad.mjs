...
const save=()=>writeFileSync(folder+'/result.json',JSON.stringify({model,cases,budgetUSD:limit,chargedUSD,stopped,rows,calls,scope:'Frozen writing plus review only; excludes search, planning, reading, full archive and conversation'},null,2));
async function request(system,input,stage){
 ...
 const body=openai?{model,store:false,service_tier:'default',reasoning:{effort:'none'},max_output_tokens:max,instructions:system,input,
  ...(stage==='write'?{tools:[{type:'function',name:'svara',description:SVARSVERKTYG.description,parameters:SVARSVERKTYG.input_schema,strict:false}],tool_choice:{type:'function',name:'svara'},parallel_tool_calls:false}:{})}
 :{model,max_tokens:max,system,messages:[{role:'user',content:input}],...(stage==='write'?{tools:[SVARSVERKTYG],tool_choice:{type:'tool',name:'svara'}}:{})};
