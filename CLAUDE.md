...
- **Deploy:** Cloudflare Pages, två projekt från samma bygge: `aktiekurs` -> `aktiekurs.pages.dev` (Delägaren) och `motparten` -> `motparten.pages.dev` (säljkursen). Deployas med **wrangler**, inte via GitHub-push:
  ...
  **`kurs-7m8.pages.dev` är en kvarliggande gammal sajt** på ett annat konto, kvar sedan flytten 2026-08-31. Den svarar 200 och ser levande ut men kör en bygg från före flytten, så en pilot som har den bokmärkt ser gamla kurser och gamla brev hur mycket vi än deployar. Kolla vilken domän en buggrapport kommer ifrån innan du felsöker koden: `curl -sD- -o/dev/null "https://<domän>/api/kurshistorik?t=AXFO&c=SE&range=5d" | grep -i cache-control` ger `max-age=21600` på den gamla och `600` eller lägre på den aktuella.
