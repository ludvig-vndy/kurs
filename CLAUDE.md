...
- **Deploy:** Cloudflare Pages, två projekt från samma bygge: `aktiekurs` -> `aktiekurs.pages.dev` (Delägaren) och `motparten` -> `motparten.pages.dev` (säljkursen). Deployas med **wrangler**, inte via GitHub-push:
  `CLOUDFLARE_ACCOUNT_ID=fbfc68e2efed9cbbe0dc0396f299e2c1 npx wrangler pages deploy dist --project-name aktiekurs --branch=main`.
  Utan `--branch=main` blir det tyst en preview, och kontot måste sättas eftersom det finns tre.
  **`kurs-7m8.pages.dev` (projektet `kurs` på VNDY-kontot `a525ec472526e7bb5e054e8f88922c50`) är släckt 2026-09-07.** Den låg kvar sedan flytten 2026-08-31, svarade 200 och såg levande ut men körde en bygg från 31 augusti, så en pilot med den bokmärkt såg gamla kurser hur mycket vi än deployade. Den kör nu ett `_redirects` som skickar `/*` vidare till `aktiekurs.pages.dev` med sökvägen kvar. Deployen gjordes från en katalog med BARA `_redirects`: `wrangler pages deploy` plockar upp `functions/` ur arbetskatalogen, så den måste köras utanför repot.
  Ligger den kvar i Supabases `uri_allow_list` (LAUNCH.md), ta bort den där också.
